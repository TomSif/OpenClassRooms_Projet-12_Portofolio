# Code Review — Interactive Comments Section (Full-Stack) — 2026-07-11

## Contexte

|                       |                                                                                          |
| --------------------- | ---------------------------------------------------------------------------------------- |
| **Niveau de review**  | N°9 (Niveaux 1 + 2 + 3 évalués)                                                          |
| **Stack**             | React 19 + TypeScript + Tailwind CSS v4 + Vite + Vitest + RTL **+ Express 5 + node:sqlite + Supertest** |
| **Review précédente** | Tip Calculator App — 4.54/5 (Niveaux 1 + 2 + 3)                                          |
| **Score FM**          | 8.0/10 (soumis en fin de Phase 2, avant l'ajout du backend)                              |
| **Durée du projet**   | ~3,5 semaines (kickoff 2026-06-17 → clôture 2026-07-11), en 3 phases : front (Phase 1-2), backend Express/SQLite (Phase 3), tests Vitest+Supertest (fin Phase 3) |

C'est le premier projet full-stack de la série : un vrai backend (Express + SQLite via `node:sqlite`), déployé en production (Render + Vercel), avec pour la première fois une suite de tests backend (Supertest, 8 tests) en plus des 26 tests front déjà connus.

---

## Progression depuis la review précédente (Tip Calculator N°8)

Le projet précédent était un projet différent (calculatrice de pourboire), donc les 3 priorités ne se "corrigent" pas littéralement ici — mais les habitudes qu'elles ciblaient sont vérifiables :

- **Priorité 1 Tip Calc (nommage d'interface de paramètres — `CalculateAmount` → `XxxParams`)** : ✅ Le réflexe tient. Toutes les interfaces de ce projet (`CommentCardProps`, `CommentInputProps`, `User`, `Reply`, `Comment`, `CommentsData`) sont nommées correctement — aucune interface de paramètres nommée comme un résultat.
- **Priorité 2 Tip Calc (composant `Input` bypassé pour un cas particulier)** : N/A, pas de terrain comparable dans ce projet.
- **Priorité 3 Tip Calc (réassignation de paramètre dans `clampMax`)** : ✅ Pas de réassignation de paramètre trouvée dans `utils/comments.ts`. En revanche, un problème voisin (shadowing de variable, pas réassignation) apparaît ailleurs — voir Priorité 3 ci-dessous.
...
Monorepo `front/` + `back/` propre. Côté front, la structure connue (`components/`, `services/`, `utils/`, `types/`) est inchangée. Côté back, séparation nette et à responsabilité unique : `db.ts` (connexion + schéma), `seed.ts` (peuplement), `app.ts` (routes Express), `server.ts` (démarrage) — ce découpage `app`/`server` a été fait spécifiquement pour rendre l'app testable par Supertest, et ça fonctionne.

Seul point à surveiller : `app.ts` contient les 6 routes CRUD dans un seul fichier (~170 lignes). Ce n'est pas un défaut à ce stade — mais si le projet grossit encore, un découpage en `routes/comments.ts` deviendra pertinent.

### 1.2 Nommage — 4/5

Conventions tenues : `camelCase`/`PascalCase` corrects, props callback en `on+verbe` (`onVote`, `onReply`, `onEdit`, `onOpening`) sans exception. Colonnes SQL en `snake_case`, mappées proprement vers du `camelCase` côté API.
...
```
Typo ("epxress"). Cosmétique, sans impact — mais visible dans les logs de prod.

Un commit sur 50 utilise un scope singulier (`feat(comment): implement comment voting`) alors que tous les autres utilisent `comments` (pluriel) — micro-incohérence, un seul cas sur 50.

### 1.3 Commits — 5/5 ⬆️ (+0.5, premier 5/5 de la série)

50 commits passés en revue un par un : **zéro irrégularité trouvée**. Aucun mot français, aucun guillemet parasite, aucun double espace, aucun scope en majuscule, aucun message vague (`wip`/`fix`/`test` seuls). Les types (`feat`, `fix`, `chore`, `refactor`, `test`, `docs`) sont tous sémantiquement corrects, y compris les cas moins évidents (`chore(back): specify Node.js version for Render`, `refactor(front/utils): remove unused buildComment/buildReply`).

C'est la première fois dans toute la série que cet axe atteint 5/5. Les reviews précédentes (Mortgage, Rest Countries, Memory Game, Tip Calculator) plafonnaient à 4.5/5 avec toujours une ou deux typos de format. Ici, rien.

### 1.4 Lisibilité — 4.5/5

Zéro `console.log` parasite (un seul `console.log` de démarrage serveur, légitime), zéro code mort, zéro commentaire redondant. Les commentaires présents (`// Arrange`, `// Act`, `// Assert` dans les tests ; `// clear the table before each test`) structurent sans paraphraser.

Les classNames Tailwind dépassent la limite des 100 caractères dans plusieurs fichiers (`CommentCard.tsx`, `CommentInput.tsx`) — cohérent avec les projets précédents où ce n'a jamais été traité comme un vrai problème de lisibilité (ce sont des chaînes utilitaires, pas de la logique). Pas de déduction pour ce point.
...
`CommentCard` est le point fort architectural du projet : un seul composant récursif gère à la fois les commentaires racine et les réponses imbriquées, sans dupliquer le markup. Les props sont bien typées, aucune logique métier n'est écrite dans le JSX (elle vit dans `utils/comments.ts` et `services/comments.ts`).

### 2.2 TypeScript — 4/5

Aucun `any` sur tout le projet (front + back), `strict: true` des deux côtés, `tsc --noEmit` propre sans erreur. Discriminated union `'up' | 'down'` bien utilisée pour le vote.
...
`deletingId` est typé `number | null`. Le `!` supprime cette possibilité sans preuve — c'est exactement le genre d'assertion que l'axe 2.2 demande de traiter plutôt qu'ignorer. Voir Priorité 3 ci-dessous pour le détail (ce point est lié à un problème de nommage plus large).

### 2.3 Gestion de l'état — 4.5/5

Bonne colocalisation : `activeReplyId` piloté depuis `App` (nécessaire, un seul input de réponse actif à la fois, potentiellement à n'importe quel niveau de profondeur) ; `isEditing`/`editContent` restent locaux à chaque `CommentCard` (pas besoin de remonter). Pas de prop drilling excessif vu la faible profondeur de l'arbre. L'optimistic UI (`handleVote`, `handleEdit`, `handleDelete` : mutation locale immédiate + appel API sans `await`, erreur rattrapée en `.catch`) est un pattern avancé bien maîtrisé et appliqué de façon cohérente sur les trois handlers.

### 2.4 DRY & réutilisabilité — 3/5

**Le point faible du projet.** La même requête SQL d'insertion est dupliquée **trois fois à l'identique** :

```ts
// app.ts — POST /comments (ligne 66)
const insert = db.prepare(`
  INSERT INTO comments (content, created_at, score, username, image_png, image_webp, parent_id, replying_to)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`);

// app.ts — POST /comments/:id/replies (ligne 98) — texte SQL identique
// seed.ts (ligne 17) — même colonnes, même ordre, formulation légèrement différente
```

Si une colonne est ajoutée ou renommée demain, il faut se souvenir de modifier trois endroits. Une fonction partagée (`insertCommentRow(db, {...})` dans `db.ts`, réutilisée par `app.ts` et `seed.ts`) éliminerait cette duplication. Voir Priorité 2.

Par ailleurs, `utils/comments.ts` (front) reste un bon exemple de factorisation réussie : `applyVote`/`applyEdit`/`applyDelete` sont extraites, testées isolément, et réutilisées à la fois par `App.tsx` et leur propre suite de tests.

---

## Évaluation par axe — Niveau 3

### 3.1 Accessibilité — 4/5

Les bons réflexes déjà acquis sur les projets précédents sont là : `alt=""` sur les icônes décoratives, `role="status"` + `aria-live="polite"` + `aria-label="Current score"` sur le score (annoncé correctement aux lecteurs d'écran à chaque vote), `<label className="sr-only">` sur le textarea, `type="button"` sur les boutons non-submit, `<dialog>` natif avec gestion clavier native.
...
```
Le timestamp ("1 month ago") est un `<h3>`. Un heading doit structurer le contenu, pas afficher une métadonnée : un utilisateur qui navigue par liste de titres (technique standard en lecteur d'écran) va entendre une série d'usernames et de dates mélangés comme s'ils étaient tous des titres de section. Un `<span>` ou un `<time>` serait plus juste.

```tsx
// CommentCard.tsx:140-141
<div
  className="md:bottom-aut absolute right-4 bottom-4 flex items-center gap-6 md:top-6 md:right-6 md:bottom-auto"
  aria-label="Buttons"
>
```
Un `aria-label` sur un `<div>` sans `role` n'est pas exposé aux technologies d'assistance (un `div` a un rôle générique, il n'entre pas dans l'arbre d'accessibilité nommé). Cet attribut ne fait donc rien ici — à supprimer, ou à remplacer par un `role="group"` si l'intention est de grouper Edit/Delete comme le fait déjà l'`aside` des votes plus haut dans le même fichier (`role="group" aria-label="vote controls"`, qui lui est correct).

### 3.2 Performance — 4/5

`key` corrects sur toutes les listes (`comment.id`, `reply.id` — jamais d'index). Pas de re-render suspect identifiable. Un point mineur : chaque route Express recrée son `db.prepare(...)` à chaque requête au lieu de préparer une fois au niveau du module — sans impact mesurable à cette échelle, mais bon à savoir pour la suite (les statements préparés sont faits pour être réutilisés).

### 3.3 Gestion des erreurs — 3.5/5

Les quatre appels réseau du front (vote, edit, delete, reply) sont tous encadrés par un `try/catch` ou un `.catch()`, avec un message affiché dans le JSX (`{error && <p>{error}</p>}`) plutôt qu'un `alert()` — la correction post-review FEM de Phase 2 tient bien. Les 403 backend (ownership) sont testés côté API mais le front ne les distingue pas d'une autre erreur réseau — un message générique ("Failed to Edit") s'affiche dans les deux cas. Pas un bug, mais une occasion manquée de message plus précis ("You can only edit your own comments").

### 3.4 Sécurité de base — 2/5 ⬇️

**Signal le plus fort de cette review.** `PUT /comments/:id` et `DELETE /comments/:id` vérifient l'ownership (`item.username !== currentUser.username → 403`). `PATCH /comments/:id/vote` ne le fait pas :

```ts
// app.ts:126-139 — aucune vérification d'auteur
...
Le front désactive juste visuellement les boutons de vote sur ses propres commentaires (`disabled={isCurrentUser}`) — une restriction cosmétique, contournable en appelant la route directement (Postman, curl). C'est exactement le principe que Thomas a lui-même identifié et appliqué sur `PUT`/`DELETE` en session 2026-07-08, puis retrouvé spontanément sur `vote` en session 2026-07-11 — mais la décision a été de ne pas corriger cette fois ("on va pas tout refaire, l'essentiel est ailleurs"). C'est un choix assumé et compris, pas un oubli — mais du point de vue de la review, l'incohérence entre les trois routes de mutation reste un vrai défaut structurel. Voir Priorité 1.

Par ailleurs : aucun secret dans le dépôt (`.env` correctement ignoré des deux côtés, jamais commité), toutes les requêtes SQL utilisent des placeholders (`?`), pas de `dangerouslySetInnerHTML`.

---

## ✅ 3 priorités pour ce projet

### Priorité 1 — Sécurité : `PATCH /comments/:id/vote` sans vérification d'ownership (axe 3.4)

**Le problème :** `PUT` et `DELETE` vérifient que `currentUser` est bien l'auteur avant de muter la ligne. `PATCH /vote` ne le fait pas — n'importe qui peut faire varier le score de n'importe quel commentaire via un appel direct à l'API.

**La correction**, calquée sur le pattern déjà écrit pour `PUT`/`DELETE` :

```ts
// app.ts — PATCH /comments/:id/vote
app.patch("/comments/:id/vote", (req, res) => {
  const id = Number(req.params.id);
  const { direction } = req.body;
  const delta = direction === "up" ? 1 : -1;
  ...
  // Note : contrairement à PUT/DELETE, voter sur SON PROPRE commentaire est
  // le cas qu'on veut bloquer ici (pas l'inverse) — le front désactive déjà
  // ce cas, mais le backend doit l'imposer :
...
Nuance importante à noter en la corrigeant : ici la règle s'inverse par rapport à edit/delete (on bloque le vote sur *son propre* commentaire, pas sur celui des autres) — à bien distinguer en l'écrivant.

---

### Priorité 2 — DRY : requête d'insertion SQL dupliquée 3 fois (axe 2.4)

**Le problème :** `POST /comments`, `POST /comments/:id/replies` et `seed.ts` répètent la même liste de 8 colonnes dans le même ordre. Une colonne ajoutée ou renommée demain doit être répercutée à la main aux trois endroits.

**La correction :**

```ts
// db.ts — nouvelle fonction partagée
export function insertCommentRow(row: {
  content: string; createdAt: string; score: number;
  username: string; imagePng: string; imageWebp: string;
  parentId: number | null; replyingTo: string | null;
}) {
  return db.prepare(`
    INSERT INTO comments (content, created_at, score, username, image_png, image_webp, parent_id, replying_to)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    row.content, row.createdAt, row.score, row.username,
    row.imagePng, row.imageWebp, row.parentId, row.replyingTo,
  );
}
```

Les trois call sites (`POST /comments`, `POST /comments/:id/replies`, `seed.ts`) n'ont plus qu'à appeler `insertCommentRow(...)`. Une seule source de vérité pour la forme de la table.

---

### Priorité 3 — Nommage/TypeScript : `deletingId` réutilisé comme nom de paramètre + assertion non-null (axes 1.2 / 2.2)
...
...
const handleDelete = (deletingId: number) => { // ← masque la variable d'état du même nom
  setComments((prev) => applyDelete(prev, deletingId))
  deleteComment(deletingId).catch(...)
}
...
onClick={() => {
  handleDelete(deletingId!) // ← assertion non-null sur un number | null
  setIsOpen(false)
}}
```

Le paramètre de `handleDelete` porte le même nom que l'état `deletingId` qu'il reçoit en argument — ça fonctionne aujourd'hui, mais c'est le genre de shadowing qui devient une source d'erreur silencieuse dès qu'une des deux définitions change de type ou de sens. Le `!` à l'appel affirme que l'état ne sera jamais `null` à ce moment précis — vrai en pratique (le bouton n'est rendu que dialogue ouvert), mais ce n'est pas garanti par le type, donc pas vérifié par le compilateur.
...
Renommer le paramètre lève l'ambiguïté ; le garde explicite remplace l'assertion et rend le cas `null` traité plutôt qu'ignoré.

---

## 📋 Backlog de progression (à traiter dans les projets suivants)

- **[Axe 1.2]** `console.log("epxress server is running")` dans `server.ts` — typo cosmétique.
- **[Axe 1.2/1.3]** Un commit (`feat(comment): implement comment voting`) utilise un scope singulier isolé contre `comments` partout ailleurs — négligeable sur 50 commits.
- **[Axe 2.4]** `PUT /comments/:id` renvoie la ligne SQL brute (`created_at`, `image_png`, snake_case) alors que `GET`/`POST` renvoient une forme normalisée en camelCase — sans impact aujourd'hui (le retour n'est pas consommé côté front), mais une incohérence de contrat d'API à surveiller si ce endpoint est un jour utilisé pour autre chose que son statut.
- **[Axe 3.2]** Les `db.prepare(...)` sont recréés à chaque requête plutôt que préparés une fois au niveau du module — sans impact mesurable ici, bon réflexe à prendre pour la suite.
- **[Axe 1.1, niveau 4 à venir]** `app.ts` regroupe les 6 routes dans un seul fichier — à surveiller si le projet grandit encore (découpage en `routes/`).
- **[Axe 4.1, niveau 4 à venir]** Le README ne mentionne pas du tout le backend (Express, SQLite, déploiement Render, Supertest) et annonce encore "24 tests" alors qu'il y en a 34 aujourd'hui (26 front + 8 back) — gros écart entre le travail réellement fait en Phase 3 et sa documentation. Le Niveau 4 n'ouvre qu'au projet 11, mais ce sera un chantier à ne pas sous-estimer.
- **[Axe 4.2, niveau 4 à venir]** `app.test.ts` contient deux `describe("PUT /comments/:id")` et deux `describe("DELETE /comments/:id")` séparés (un par cas succès/403) — à fusionner en un seul `describe` par route avec plusieurs `it`, point déjà identifié par Thomas lui-même mais pas encore fait.

---

## 📊 Score global

| Axe | Rest Countries (N6) | Memory Game (N7) | Tip Calculator (N8) | **Interactive Comments (N9)** | Δ |
|-----|:---:|:---:|:---:|:---:|:---:|
| 1.1 Structure | 4 | 4 | 4.5 | **4.5** | = |
| 1.2 Nommage | 4 | 4 | 4.5 | **4** | ⬇️ -0.5 |
| 1.3 Commits | 4 | 3.5 | 4.5 | **5** | ⬆️ +0.5 |
| 1.4 Lisibilité | 4 | 3.5 | 4.5 | **4.5** | = |
| 2.1 Composants | 4 | 4 | 4.5 | **4.5** | = |
| 2.2 TypeScript | 4 | 4 | 4.5 | **4** | ⬇️ -0.5 |
| 2.3 Gestion d'état | 3.5 | 4 | 5 | **4.5** | ⬇️ -0.5 |
| 2.4 DRY | 3.5 | 3.5 | 4 | **3** | ⬇️ -1 |
| 3.1 Accessibilité | — | 3.5 | 5 | **4** | ⬇️ -1 |
| 3.2 Performance | — | 4 | 4.5 | **4** | ⬇️ -0.5 |
| 3.3 Gestion des erreurs | — | 3 | 4 | **3.5** | ⬇️ -0.5 |
| 3.4 Sécurité | — | 5 | 5 | **2** | ⬇️ -3 |
| **Moyenne** | — | 3.9 | 4.54 | **3.96** | ⬇️ -0.58 |

**Verdict :** La moyenne recule par rapport au sommet de Tip Calculator, mais ce recul se lit différemment axe par axe. Les axes 1.1, 1.3 et 2.1 progressent ou tiennent — les commits atteignent pour la première fois 5/5. Le recul vient de deux endroits précis, pas d'un relâchement général : la sécurité (le vote non protégé, un vrai défaut structurel malgré la décision consciente de ne pas le corriger maintenant) et le DRY backend (SQL dupliqué). Ce sont deux terrains neufs — premier backend, première fois qu'un vrai contrat API doit être tenu cohérent entre plusieurs routes — pas une régression sur des acquis déjà solides.

---

## Note finale

C'est le premier projet full-stack de la série, et il tient : un vrai backend Express + SQLite, un modèle relationnel construit à la main depuis zéro (schéma, seed, requêtes préparées), une API REST complète avec gestion d'ownership sur deux routes sur trois, un déploiement en production sur deux plateformes différentes avec deux bugs de prod diagnostiqués et corrigés en autonomie, et pour la première fois une suite de tests backend (8 tests Supertest) qui couvre explicitement un cas d'échec attendu (`403`) — un type de test jamais écrit avant sur ce parcours.

Les 50 commits sans une seule irrégularité de format sont un signal à noter : c'est la première fois que cet axe est parfait, après quatre reviews consécutives où une typo ou un scope mal formé traînait toujours quelque part.

La faille de sécurité sur le vote est le point le plus important de cette review, pas parce qu'elle a été manquée, mais parce qu'elle a été **vue et nommée en session** (deux fois : sur delete puis sur vote) avant d'être laissée de côté par choix. C'est le bon réflexe de sécurité — la même vigilance appliquée à trois routes plutôt que deux serait la version complète de ce réflexe.

**Le Niveau 4 (documentation, tests avancés, cohérence ESLint/Prettier, architecture globale) ouvre au projet 11 — dans deux projets. Le README de ce projet, qui ne reflète encore que la Phase 2 front, sera le premier chantier concret à ce moment-là.**
